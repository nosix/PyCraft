...
class Scratch(object):
    ...
    def _parse_sensorupdate(self, msg):
        ...
        curr_seg = b'' # current segment (i.e. key or value) being formed
        numq = 0 # number of double quotes in current segment
        for seg in update.split(b' ')[:-1]: # last char in update is a space
            numq += seg.count(b'"')
            curr_seg += seg
    ...
    def _write(self, data):
        ...
        while total_sent < length:
            try:
                sent = self.socket.send(bytes(data[total_sent:], 'UTF-8'))
            except socket.error:
                self.connected = False
                raise ScratchError from socket.error
            if sent == 0:
                self.connected = False
